...
Quizler Arena uses open-source packages declared in `package.json`, including:
...
I do not claim authorship over external vocabulary lists. They are included as inputs to the application's data pipeline. Anyone reusing those datasets outside this project should confirm the original upstream source and license that applies to the specific list or package.

The Jeopardy expansion packs, validation rules, bank-building workflow, replayability system, and runtime integration are part of the Quizler Arena project. The README describes the 5,600-clue expansion as curated rather than claiming that every factual clue was written from scratch without research or tool assistance.
...
AI-assisted tools were used during parts of implementation, debugging, content drafting, and visual experimentation. Generated output was treated as material to inspect, test, revise, or replace rather than as automatically trusted application data.
